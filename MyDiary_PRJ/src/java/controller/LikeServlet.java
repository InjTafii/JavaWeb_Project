...
public class LikeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean check = !foundDiary.isFavourite();
        foundDiary.setFavourite(check);
        Diary editResult = Helper.diaryRepository()
                    .update(foundDiary.getId(), foundDiary);
        HttpSession session = request.getSession();
        User u = (User) session.getAttribute("user");
        session.setAttribute("diaryList", Helper.diaryRepository().getFavourite(u != null ? u.getId() : null));
